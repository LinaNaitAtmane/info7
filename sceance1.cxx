...
Piece get_square(int n, int m, Piece T[64])
{
    if ((n < 0) || (n >= 8) || (m < 0) || (m >= 8))
    {
        cout << "Erreur";
...
void set_square(int n, int m, Piece T[64], Piece x)
{
    if (n >= 0 && n < 8 && m >= 0 && m < 8)
        T[n * 8 + m] = x;
}
...
void move_piece(int i1, int j1, int i2, int j2, Piece T[64])
{
    T[i2 * 8 + j2] = T[i1 * 8 + j1];
    T[i1 * 8 + j1] = vide;
}
